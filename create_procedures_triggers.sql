...
CREATE PROCEDURE CreateOrder (
    IN user_id BIGINT,           -- 用户ID
    IN input_product_id INT,     -- 产品ID
    IN order_quantity INT,       -- 订单数量
    OUT new_order_id BIGINT      -- 新订单ID
)
BEGIN
    DECLARE stock INT;           -- 产品库存
    DECLARE product_price DECIMAL(10,2);  -- 产品单价

    -- 检查产品库存是否足够
    SELECT stock, price INTO stock, product_price
    FROM products
    ...
    IF stock >= order_quantity THEN
        ...
        VALUES (user_id, NOW(), product_price * order_quantity);

        -- 获取新创建的订单ID
        SET new_order_id = LAST_INSERT_ID();

        -- 创建订单项
        INSERT INTO order_items (order_id, product_id, quantity, price, merchant_id, status)
        VALUES (new_order_id, input_product_id, order_quantity, product_price, user_id, 'PENDING');
    ELSE
        -- 库存不足，返回错误信息
